...
int MoreThanHalfNum_Solution1(int* numbers, int length)
{
    ...
    int start = 0;
    int end = length - 1;
    // numbers中index左侧的数小于等于右侧的数
    int index = Partition(numbers, length, start, end);
    // 如果index==middle，那么这个中间的数就是要找的数
    while(index != middle)
    {
        if(index > middle)
        {
...
// 当我们遍历到下一个数字的时候，如果下一个数字和我们之前保存的数字相同，则次数+1；
// 如果不同，则次数-1，如果次数为0，那么我们需要保存下一个数字，并把次数设为1.
// 由于我们要找的数字出现的次数比其他所有数字出现的次数之和还要多，那么要找的数字肯定是
// 最后一次把次数设为1时对应的数字。
