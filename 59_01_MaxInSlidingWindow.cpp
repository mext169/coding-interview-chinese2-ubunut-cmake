...
/**
 *
 * @param num 输入数组，保存在了vector里面
 * @param size 滑动窗口的大小
 * @return
 */
vector<int> maxInWindows(const vector<int>& num, unsigned int size)
{
    // 每个滑动窗口里面的最大值组成的vector
    vector<int> maxInWindows;
    // 数组元素个数得大于滑动窗大小 且 滑动窗口大于1
    if(num.size() >= size && size >= 1)
    {
        // 双端队列
        deque<int> index;

        // 这个循环结束后，index的第一个数字是前size个数中最大的
        for(unsigned int i = 0; i < size; ++i)
        {
            // 第一次进来，index是空的，先把第一个数放进来
            // 后面进来，index不是空的，来的数字比index最后面的数要大，就把后面的数弹出，然后再把这个大数push_back
